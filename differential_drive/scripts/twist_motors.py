...
class TwistToMotors(object):
	def __init__(self, node_name='twist_to_motors'):
    		...
    		self.pub_clrmap = rospy.Publisher('/slamware_ros_sdk_server_node/clear_map', ClearMapRequest, latch=True, queue_size=1)
    		...
    		# MAP COMMAND
    		self.msg_clrmap = ClearMapRequest()
	...
	def spinOnce(self):		
		if self.wh == False and self.auto == True and self.manual == False:		
	   		print("Auto --> linear & Angular Velocity")
	   		print(self.dx, self.dr)
	   		if self.dx > 1.05:
	   			self.dx = 1.05
	   		elif self.dx < -1.05:
	   			self.dx = -1.05
	   		...
	   		while not rospy.is_shutdown() and cnt_true ==0:
	   			print("enterring")
	   			...
	   			if lft_whl_rps < 0.0 and rgt_whl_rps >= 0.0:
	   				self.lr_wh_vel.data = [abs(rgt_whl_rps), abs(lft_whl_rps), 0, 1]
	   			...
	   			if rgt_whl_rps < 0.0 and lft_whl_rps >= 0.0:
	   				self.lr_wh_vel.data = [abs(rgt_whl_rps), abs(lft_whl_rps), 1, 0]
	   				self.pub_motor_vel.publish(self.lr_wh_vel)
	   				self.ticks_since_target = self.ticks_since_target + 1
	   			
	   			if rgt_whl_rps < 0.0 and lft_whl_rps < 0.0:
	   				self.lr_wh_vel.data = [abs(rgt_whl_rps), abs(lft_whl_rps), 1, 1]
	   				self.pub_motor_vel.publish(self.lr_wh_vel)
	   				self.ticks_since_target = self.ticks_since_target + 1
	   					   			
	   			if rgt_whl_rps > 0.0 and lft_whl_rps > 0.0:	
	   				self.lr_wh_vel.data = [abs(rgt_whl_rps), abs(lft_whl_rps), 0, 0]
	   				self.pub_motor_vel.publish(self.lr_wh_vel)
	   				self.ticks_since_target = self.ticks_since_target + 1
	   				
	   			if rgt_whl_rps == 0.0 and lft_whl_rps == 0.0:
	   				self.lr_wh_vel.data = [0, 0, 0, 0]
	   				self.pub_motor_vel.publish(self.lr_wh_vel)
	   				self.ticks_since_target = self.ticks_since_target + 1
	   			cnt_true= cnt_true + 1
	   		print(self.lr_wh_vel.data[0], self.lr_wh_vel.data[1])
	...
	def twistCallback(self, msg):
   		...
   		if self.auto == True and self.amrstop == False:  
   			print("auto lin vel",self.dx)
   			print("auto ang vel",self.dr) 			
   			self.dx = ((msg.linear.x * 1.0) *(self.lnvelSLD/255))
   			self.dr = ((msg.angular.z * 1.0) *(self.anvelSLD/255))
   		...
   		elif self.auto == True and self.amrstop == True:
   			self.cmdvel.linear.x = 0.0
   			self.cmdvel.angular.z = 0.0
   			self.dx = self.cmdvel.linear.x
   			self.dr = self.cmdvel.angular.z   			
   			self.ticks_since_target = 0
   			self.amrstop = False
	...
	def manual_mode(self):
		...
		#self.cmdvel.linear.x = 0.0
		#self.cmdvel.angular.z = 0.0
		#self.pub_vel.publish(self.cmdvel)		
		print("manual_mode")	
			
	def clear_map(self):
		self.msg_clrmap.kind.kind = 0
		self.pub_clrmap.publish(self.msg_clrmap)
	...
	def calc_pub_pulse(self,message):		
		if message.data == 16 and self.auto == True:
			self.clear_map()
		elif message.data == 17 and self.auto == True:
			self.enable_map()
		elif message.data == 18 and self.auto == True:
			self.disable_map()
		if message.data == 14:
			self.auto_mode()
